...
typedef RT_MODEL_pid_q_torsion_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * pid_q_torsion(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(pid_q_torsion_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(pid_q_torsion_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://pid_q_torsion:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: pid_q_torsion -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(pid_q_torsion_DW.HILReadEncoderTimebase_Task,
        (t_clock) pid_q_torsion_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
