  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_rotpen_swingup_student_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (q_rotpen_swingup_student_DW.HILReadEncoderTimebase_Task, (t_clock)
         q_rotpen_swingup_student_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
