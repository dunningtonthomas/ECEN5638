...
typedef RT_MODEL_control_experiment_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * control_experiment(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(control_experiment_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: control_experiment -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
