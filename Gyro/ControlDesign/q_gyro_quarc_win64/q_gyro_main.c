...
typedef RT_MODEL_q_gyro_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_gyro(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_gyro_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://q_gyro:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_gyro -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
